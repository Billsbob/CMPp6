    ...
    for name in image_names:
        ...
        if target_dtype is None:
            ...
            if current_dtype != target_dtype:
                print(
                    f"Error: Image {name} has bit depth {current_dtype}, expected {target_dtype}. All images must have matching bit depth.")
                return None
            if current_channels != target_channels:
                print(
                    f"Error: Image {name} has {current_channels} color channels, expected {target_channels}. All images must have matching raw color.")
                return None
        ...
        if target_shape is None:
            ...
            print(
                f"Error: Image {name} has shape {data.shape}, expected {target_shape}. All images must have the same dimensions after transformation to be stacked.")
            return None
