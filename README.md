...
For clustering, begin by selecting only those probes you are interested in.
  ...
  To produce histograms, select One mask and any number of images. Histograms will be listed in the Graphs section and a JSON saved in the Graphs folder.  You can select
  which histograms to view together and save a .csv and snapshot png.
  You can also produce a joint plot with up to three comparisons between two images under a specified mask. 

  Fine-tuning clustering can be accomplished by using spatial awareness in the ISODATA parameters window. By checking "Include Coordinates" you can set the 'cost' of clustering in either the x or y dimenstion.
